...
"@ | Set-Content -Path $runnerPath -Encoding ASCII
        ...
        $eventLines = @()
        if ($lastLogPath) {
            $eventPattern = "CDB_POST_PAYLOAD_ALLOC_RETURN|CDB_POST_PAYLOAD_ALLOC20_RETURN|CDB_FRIDA_MATCHED_ALLOC20_RETURN|CDB_FRIDA_MATCHED_ALLOC20_STACK|CDB_NEAR_MISS_ALLOC30_RETURN|CDB_NEAR_MISS_ALLOC30_STACK|CDB_PAYLOAD_RELEASE_ENTER|CDB_PAYLOAD_RELEASE_STACK|CDB_PAYLOAD_AFTER|CDB_PAYLOAD_RTLFREEHEAP_ENTER|CDB_BAD_CLEANUP_RET|CDB_EXACT_REUSE_RUNTIME|CDB_WRITE_TO_REUSED_SLOT"
            $eventLines = @(
